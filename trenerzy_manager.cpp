...
{
    if (index >= 0 && index < (int) trenerzy.size())
        trenerzy.erase(trenerzy.begin() + index);
}
std::vector<Trener> &TrenerzyManager::getTrenerzy()
{
    return trenerzy;
}
const Trener &TrenerzyManager::getTrener(int index) const
{
    return trenerzy.at(index);
}
...
bool TrenerzyManager::zapiszDoPliku(const QString &sciezkaPliku) const
{
    ...
    for (const Trener &t : trenerzy)
        out << t.getId() << "," << QString::fromStdString(t.getImie()) << ","
