...
void ZajeciaManager::dodajZajecia(const Zajecia &z)
{
    zajecia.push_back(z);
}
void ZajeciaManager::usunZajecia(int index)
{
    if (index >= 0 && index < (int) zajecia.size())
        zajecia.erase(zajecia.begin() + index);
}
std::vector<Zajecia> &ZajeciaManager::getZajecia()
{
    return zajecia;
}
...
bool ZajeciaManager::zapiszDoPliku(const QString &sciezkaPliku) const
{
    ...
    for (const Zajecia &z : zajecia) {
        out << z.getIdTrenera() << "," << z.getTrenerImieNazwisko() << ","
            << z.getStart().toString(Qt::ISODate) << "," << z.getEnd().toString(Qt::ISODate) << ","
            << z.getSala() << "," << z.getLimitMiejsc() << "," << z.getWolnychMiejsc() << "\n";
    }
    plik.close();
    return true;
}
